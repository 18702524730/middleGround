		...
		initRouters(){
				...
				for (var i = 0; i < routes.length; i++) {
					if (routes[i].hidden !== true) {
						...
						tempObj.isAllowed = permission.indexOf(module.meta.permission) !== -1;
						tempObj.children = [];
						for (var j = 0; j < menus.length; j++) {
							if (menus[j].hidden !== true && menus[j].children && menus[j].children.length) {
								...
								tempChildObj.isAllowed = permission.indexOf(menu.meta.permission) !== -1;
								...
								for (var k = 0; k < pages.length; k++) {
									if (pages[k].hidden !== true) {
										...
										tempPageObj.isAllowed = permission.indexOf(page.meta.permission) !== -1;
