<template>
	<div class="upload_wrap">
		...
		<div class="file-uploader" :class="{'file-uploader-onbg': startShow}" @click="addFile">
			...
			<div v-if="uploadedFileData.url">
				<div class="success successImg" v-if="isPdf(suffix)" @mouseenter="dialogImageUrl=pdfPreviewImg">
					<img :src="pdfPreviewImg">
					<div class="mask"></div>
					<div class="hoveraction"><i class="el-icon-view" @click="dialogImgVisible=true"></i><i class="el-icon-delete2" @click.stop="handleRemove()"></i></div>
				</div>
    ...
    <form onsubmit="return false;">
		<el-dialog v-model="dialogImgVisible" :custom-class="pdfPreviewImgs.length && pdfPreviewImgs.length>1 ? 'previewDialog previewDialog_trans' : 'previewDialog'" :modal-append-to-body="true" :lock-scroll="true" append-to-body="true" :top="pdfPreviewImgs.length && pdfPreviewImgs.length>1 ? '50vh' : isDialog ? '0' : '15vh'" :modal="isDialog? false: true" ref="previewDialog">
			...
			<div v-else-if="isDialog" style="height: 500px;overflow: auto;">
...
<script>
	...
	export default {
		...
		props:{
			...
			postAction:{
				type: String,
				default: rootUrl + '/api/file/uploadStandby'
			},
			name: {
				type: String,
				default: 'fileData'
