<template>
	<div class="note_memo" v-loading="loading">
		 <div class="note_table">
			...
			<!-- 没有数据时候内容显示 -->
			<div class="note_tab clearfix" v-if="!noteData.length">
				<p class="td_1 fl">暂无记录</p>
				<p class="td_2 fl"></p>
				<p class="td_3 fl"></p>
			</div>
...
<script>
    ...
    methods: {
    	info(){
    		var self = this;
	    	self.loading = true;
	    	this.$http.get(noteUrl+'/' + this.detailData.order_sn)
	    	.then((resp) => {
...
<style lang="less">
	.note_memo{background-color:#fff;padding:20px;border:1px solid #dfe2e5;border-top: 0;
		.note_table{
