<template>
  <div class="workCommonList clientServiceOrder">
	  <div class="condition clearfix">
	  	...
	  	<div class="form_control"><el-input v-model="searchObj.truename" placeholder="客户姓名"></el-input></div>
	  	<div class="form_control">
				<el-select v-model="searchObj.newsNumber" clearable placeholder="排序方式">
					<el-option
			...
			<div class="form_submit">
				<button class="submit" @click="findCustomerListFn(); currentPage = 1;">搜索</button>
	  ...
	  <div class="result">
        ...
        <tbody class="tr-color">
          <tr v-for="(item, index) in messageList" v-show="!loading && messageList.length" >
	    ...
	    <!-- <div class="my_loading" v-show="loading" >
	    </div> -->
	    <div class="tb_pagination" v-if="!loading && messageList.length">
	    	<el-pagination
  ...
  export default {
    ...
    data() {
      return {
      	...
      	//排序列表
      	sortList: [
      		{name: '按被异议风险由多到少', id:1},
      		{name: '按被撤三风险由多到少', id:2},
      		{name: '按变更风险由多到少', id:3},
      		{name: '按到期风险由多到少', id:4},
      		{name: '按可异议情报由多到少', id:5},
      		{name: '按可撤三情报由多到少', id:6},
      		{name: '按未续展撤销情报由多到少', id:7},
      	], 
      	distributeIsAllowed: false,  //是主管 
      }
    },
    mounted() {
    ...
    methods: {
      ...
      findCustomerListFn () {
        let params = {
          ...
          memberPhone: this.searchObj.memberSn,
          nickname: this.searchObj.nickname,
          truename: this.searchObj.truename,
          newsNumber: this.searchObj.newsNumber
        }
        this.loading = true;
      ...
      // 翻页
      currentPageFn () {
      	this.findCustomerListFn();  //默认查询搜索记录
      }
    },
    filters: {
