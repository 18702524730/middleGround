    ...
    <div class="form">
      <template v-if="status == 1">
        ...
        <div v-if="detailInfo.pasInvoiceInfoFormList">
          ...
          <!-- 上传申请材料 -->
            <div class="basic_lists"  v-if="detailInfo.pasFlowFeedbackMap[1] || detailInfo.currentFlowStage==1">
              <div class="basictitle">
          ...
          <!-- 上传实质请求审核结果 -->
          <div v-if="detailInfo.pasFlowFeedbackMap[6] || detailInfo.currentFlowStage==6">
            <div class="basic_lists">
              <div class="basictitle">
                <h3 class = 'fl'>上传实质请求审核通知</h3>
                <el-button type="primary" @click="uploadMaterial('u','ssresult')" v-if="detailInfo.currentFlowStage==6" class = 'fr'>反馈审核结果</el-button>
                
              </div>
              <div class="basic_category clearfix" v-if="detailInfo.pasFlowFeedbackMap[6]">
                <div class="firstaudit clearfix" v-for="(item,key) in detailInfo.pasFlowFeedbackMap[6]" :key="key">
                    <ul class="fl">
                      <li>
                        <p class="tit">实质请求审核结果：</p>
                        <p class="txt">{{'通过'}}</p>
                      </li>
                      <li v-if="item.pasFiles">
                        <p class="tit">官文：</p>
                    ...
                    <div class="fr"><el-button type="primary" @click="uploadMaterial('c','ssresult',item)">修改</el-button></div>
                </div>
              </div>
            </div>
          </div>
          <!-- 反馈实质审查结果 -->
          <div class="basic_lists" v-if="detailInfo.pasFlowFeedbackMap[7] || detailInfo.currentFlowStage==7">
            <div class="basictitle">
              <h3 class = 'fl'>反馈实质审查结果</h3>
              <el-button type="primary" @click="uploadMaterial('u','standby4ss')" v-if="detailInfo.currentFlowStage==7" class = 'fr'>反馈实质审查结果</el-button>
            </div>
            <div class="basic_category clearfix" v-if="detailInfo.pasFlowFeedbackMap[7]">
              <div class="firstaudit clearfix" v-for="(item,index) in detailInfo.pasFlowFeedbackMap[7]" :key="index">
                <ul  class="fl">
                  <li>
                    <p class="tit">实质审查结果：</p>
                    <p class="txt">{{item.substantReviewResult|trialReviewResultFormat}}</p>
                  </li>
                  <li v-if="item.substantReviewResult==1">
                  ...
                  <li v-if="item.substantReviewResult==2">
                    <p class="tit nopay" >补正/审查截止日期：</p>
                    <p class="txt nopay">{{item.supplementReviewDeadline|dateFormat}}</p>
                  </li>
                  <li v-if="item.substantReviewResult==3">
                    <p class="tit nopay">驳回复审截止日期：</p>
                    <p class="txt nopay">{{item.rejectReviewDeadline|dateFormat}}</p>
                  </li>
                  <li v-if="item.pasFiles">
              ...
              <div class="fr">
                <el-button type="primary" @click="uploadMaterial('c','standby4ss',item)" >修改</el-button>
              </div>
              </div>
          ...
          <!-- 补正/审查(实审) -->
          <div class="basic_lists" v-if="detailInfo.pasFlowFeedbackMap[8] || detailInfo.currentFlowStage==8">
            <div class="basictitle">
              <h3 class = 'fl'>补正/审查（实审）</h3>
              <el-button type="primary" @click="uploadMaterial('u','standby5ss')" v-if="detailInfo.currentFlowStage==8" class = 'fr'>上传补正材料</el-button>
            </div>
            <div class="basic_category clearfix" v-if="detailInfo.pasFlowFeedbackMap[8]">
              <div class="firstaudit clearfix" v-for="(item,key) in detailInfo.pasFlowFeedbackMap[8]" :key="key">
                <ul class="fl">
                  <li>
                    <p class="tit">递交日期：</p>
                    <p class="txt">{{item.submitTime|dateFormat}}</p>
                ...
                <div class="fr">
                  <el-button type="primary" @click="uploadMaterial('c','standby5ss',item)" >修改</el-button>
                </div>
              </div>
            </div>
          </div>
      ...
      <!-- 实质请求审核结果 -->
      <el-dialog :title="ssresult_title" :visible.sync="is_ssresult" class="state_preview">
          <el-form label-width="130px">
            <el-form-item  label="实质请求审核结果">
                <el-radio v-model="ssresult_Datas.substantReqAuditResult" :label="1">通过</el-radio>
              </el-form-item>
              <el-form-item class="material" label="官文" prop="material">
                <div class="upload_component" style="margin-bottom:20px;">
                  <pasuploader @complete="setUploadedMaterial" @resetUploader="resetUploader" :fileName="ssresult_name"  :canPreview="false" :postAction="'iprp_middleground/api/file/upload'" :url="ssresult" :valueName="'ssresult'"   :isDialog="true"></pasuploader>
                  <div class="upload_tip">
                    <div v-if="ssresult_Data.length>0">
                      <p v-for="(file,index) in ssresult_Data" :key="index">已上传：{{decodeURIComponent(file.fileName)}} <span class="downloadbtn" @click="commonDel('ssresult',file)">删除</span></p>
                    </div>
                    <!-- <p class="format">备注：此阶段官文包括但不限于《授予外观设计专利权通知书》、《办理登记手续通知书》，文件格式仅支持pdf</p> -->
                  </div>
                </div>
              </el-form-item>
          </el-form>
        <div class="btnbox">
          <el-button size="large" type="primary" @click="commonSubmitu('ssresult')" v-if="ischange=='u'">确定</el-button>
          <el-button size="large" type="primary" @click="commonSubmitc('ssresult')" v-if="ischange=='c'">确定</el-button>
          <span style="padding-left:50px;"></span>
          <el-button size="large" type="default" @click="commonCancel('ssresult')">取消</el-button>
        </div>
      </el-dialog>
      <!-- 修改实质审查 -->
      <el-dialog :title="standby4ss_title" :visible.sync="is_standby4ss" class="state_preview">
        <div class="reviseapplybox">
          <el-form ref="standby4ss_Datas" label-width="120px" :model="standby4ss_Datas">
            <el-form-item class="submitdate" label="实质审查结果" prop="radio">
              	<el-radio v-model="standby4ss_Datas.substantReviewResult" :label="1" :disabled="ismodifyss">通过</el-radio>
                <el-radio v-model="standby4ss_Datas.substantReviewResult" :label="2" :disabled="ismodifyss">补正/审查意见</el-radio>
				        <el-radio v-model="standby4ss_Datas.substantReviewResult" :label="3" :disabled="ismodifyss">驳回</el-radio>
            </el-form-item>
            <div v-if="standby4ss_Datas.substantReviewResult==1">
              <el-form-item class="submitdate" label="办登费缴纳截止日期" prop="boardFeeDeadline">
                <el-date-picker v-model="standby4ss_Datas.boardFeeDeadline" type="date" placeholder="选择日期时间"></el-date-picker>
              </el-form-item>
              <el-form-item class="submitdate" label="第几年度年费" prop="whichYear">
                <!-- <el-input type="number" v-model="standby4ss_Datas.whichYear" placeholder="请填写年度" :min="1" :max="20"></el-input> -->
                <el-select v-model="standby4ss_Datas.whichYear" placeholder="请选择">
                  <el-option
              ...
              <el-form-item class="material" label="官文" prop="material">
                <div class="upload_component" style="margin-bottom:20px;">
                  <pasuploader @complete="setUploadedMaterial" @resetUploader="resetUploader" :fileName="standby4ss_name" :canPreview="false" :postAction="'iprp_middleground/api/file/upload'" :url="standby4ss" :valueName="'standby4ss'"   :isDialog="true"></pasuploader>
                  <div class="upload_tip">
                    <div v-if="standby4ss_Data.length>0">
                      <p v-for="(file,index) in standby4ss_Data" :key="index">已上传：{{decodeURIComponent(file.fileName)}} <span class="downloadbtn" @click="commonDel('standby4ss',file)">删除</span></p>
                    </div>
                  </div>
                </div>
              </el-form-item>
            </div>
            <div v-if="standby4ss_Datas.substantReviewResult==2">
              <el-form-item class="submitdate" label="补正/审查截止日期" prop="subdate">
                <el-date-picker v-model="standby4ss_Datas.supplementReviewDeadline" type="date" placeholder="选择日期时间"></el-date-picker>
              </el-form-item>
              <el-form-item class="material" label="官文" prop="material">
                <div class="upload_component" style="margin-bottom:20px;">
                  <pasuploader @complete="setUploadedMaterial" @resetUploader="resetUploader" :fileName="standby4ss_name" :canPreview="false" :postAction="'iprp_middleground/api/file/upload'" :url="standby4ss" :valueName="'standby4ss'"   :isDialog="true"></pasuploader>
            ...
            <div v-if="standby4ss_Datas.substantReviewResult==3">
              <el-form-item class="submitdate" label="驳回复审截止日期" prop="subdate">
                <el-date-picker v-model="standby4ss_Datas.rejectReviewDeadline" type="date" placeholder="选择日期时间"></el-date-picker>
              </el-form-item>
              <el-form-item class="material" label="官文" prop="material">
                <div class="upload_component" style="margin-bottom:20px;">
                  <pasuploader @complete="setUploadedMaterial" @resetUploader="resetUploader" :fileName="standby4ss_name" :canPreview="false" :postAction="'iprp_middleground/api/file/upload'" :url="standby4ss" :valueName="'standby4ss'"   :isDialog="true"></pasuploader>
        ...
        <div class="btnbox">
          <el-button size="large" type="primary" @click="commonSubmitu('standby4ss')" v-if="ischange=='u'">确定</el-button>
          <el-button size="large" type="primary" @click="commonSubmitc('standby4ss')" v-if="ischange=='c'">确定</el-button>
          <span style="padding-left:50px;"></span>
          <el-button size="large" type="default" @click="commonCancel('standby4ss')">取消</el-button>
        </div>
      </el-dialog>
      <!-- 修改补正/审查实审 -->
      <el-dialog :title="standby5ss_title" :visible.sync="is_standby5ss" class="state_preview">
          <div class="reviseapplybox">
            <div class="bzbox">
              <el-form ref="standby5ss_Datas" label-width="100px"  :model="standby5ss_Datas">
                <el-form-item class="submitdate" label="递交日期" prop="submitTime">
                  <el-date-picker v-model="standby5ss_Datas.submitTime" type="date" placeholder="选择日期时间"></el-date-picker>
                </el-form-item>
                <el-form-item class="material" label="补正资料" prop="material">
                  <div class="upload_component" style="margin-bottom:20px;">
                    <pasuploader @complete="setUploadedMaterial" @resetUploader="resetUploader" :fileName="standby5ss_name" :canPreview="false" :postAction="'iprp_middleground/api/file/upload'" :url="standby5ss" :valueName="'standby5ss'"   :isDialog="true"></pasuploader>
                    <div class="upload_tip">
                      <div v-if="standby5ss_Data.length>0">
                        <p v-for="(file,index) in standby5ss_Data" :key="index">已上传：{{decodeURIComponent(file.fileName)}} <span class="downloadbtn" @click="commonDel('standby5ss',file)">删除</span></p>
                      </div>
                    </div>
                  </div>
                </el-form-item>
          ...
          <div class="btnbox">
            <el-button size="large" type="primary" @click="commonSubmitu('standby5ss')" v-if="ischange=='u'">确定</el-button>
            <el-button size="large" type="primary" @click="commonSubmitc('standby5ss')" v-if="ischange=='c'">确定</el-button>
            <span style="padding-left:50px;"></span>
            <el-button size="large" type="default" @click="commonCancel('standby5ss')">取消</el-button>
          </div>
      </el-dialog>
...
export default {
  ...
  methods: {
    ...
    torevise() {
      ...
      if(step==21){
        this.$router.push({name: "inventuploadApplyRevise",params: { state: this.caseSysNo,type:2,}});
      }
      if(step==22){
        this.$router.push({name: "inventuploadNoticeRevise",params: { state: this.caseSysNo,type:2,}});
      }
      if(step==23){
        this.$router.push({name: "inventapplyFeeRevise",params: { state: this.caseSysNo,type:2,}});
      }
      if(step==24){
        this.$router.push({name: "inventfirstAuditRevise",params: { state: this.caseSysNo,type:2,}});
      }
      if(step==25){
        this.$router.push({name: "inventtoBeFilledRevise",params: { state: this.caseSysNo,type:2,}});
      }
      if(step==26){
        this.$router.push({name: "inventrejectedRevise",params: { state: this.caseSysNo,type:2,}});
      }
      if(step==27){
        this.$router.push({name: "substantiveReqRevise",params: { state: this.caseSysNo,type:2,}});
      }
      if(step==28){
        this.$router.push({name: "substantiveAuditRevise",params: { state: this.caseSysNo,type:2,}});
      }
      if(step==29){
        this.$router.push({name: "toBeFilledSubRevise",params: { state: this.caseSysNo,type:2,}});
      }
      if(step==210){
        this.$router.push({name: "rejectedSubRevise",params: { state: this.caseSysNo,type:2,}});
      }
      if(step==211){
        this.$router.push({name: "inventbdfPayRevise",params: { state: this.caseSysNo,type:2,}});
      }
      if(step==212){
        this.$router.push({name: "inventyearPayRevise",params: { state: this.caseSysNo,type:2,}});
      }
    },
    uploadMaterial(uc,t,n) {
      if(uc=='u'){
        this.ischange = 'u';
        // this[t+'_Datas'] = Object.assign({},t+'_Data');
        switch(t){
          case 'standby1':
