	  ...
	  <div class="result">
        ...
        <thead>
          <tr class="thead">
			...
			<th class="smWidth">实质审查结果</th>
			<th class="smWidth">办登费缴纳截止日期</th>
        ...
        <tbody class="tr-color">
          <tr v-for="(item, index) in dataList" v-show="!loading && dataList.length">
			...
			<td>{{item.substantReviewResult|trialReviewResultFormat}}</td>
			<td class="nopay">{{item.boardFeeDeadline|dateFormat}}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      view(item){
      	this.$router.push({name: 'inventbdfPayDetail', params: {id: item.caseSysNo,step:211}})
      },
