  ...
  <div class="workCommonList clientServiceOrder">
	  ...
	  <div class="condition clearfix">
		...
		<div class="form_submit">
			...
			<button v-if="isexport" @click="exportlist">导出</button>
		</div>
	  </div>
	  ...
	  <div class="result">
        ...
        <thead>
          <tr class="thead">
			...
			<th class="smWidth">第几年度年费</th>
			<th class="smWidth">距截止日期</th>
			<th class="smWidth">截止日期</th>
			<th class="minWidth">监控状态</th>
        ...
        <tbody class="tr-color">
          <tr v-for="(item, index) in dataList" v-show="!loading && dataList.length">
			...
			<td class="nopay alignc">{{item.whichYear}}</td>
			<td>{{item.deadlineDay}}天</td>
			<td class="nopay">{{item.deadline|dateFormat}}</td>
			<td class="nopay">{{item.monitorState|monitorStateFormat}}</td>
...
<script>
	...
	const workOrderListUrl = './api/findPasYearFeeMonitorList';
	const dataObj = {//条件数据
		// contractNo:null,
		patentName:null,
		applicant: null,
  ...
  export default {
	...
	computed:{
		isexport(){
			...
			return arr.indexOf('pas_invent_flow_year_fee_paid_export')>-1
		}
	},
    methods: {
		exportlist(){
			...
			if (this.applytime) {
				datas.deadlineStart = this.applytime[0] ? new Date(this.applytime[0]).getTime() : null;
				datas.deadlineEnd = this.applytime[1] ? new Date(this.applytime[1]).getTime() + 86390000 : null;
			}
			let ret = Object.assign({},this.dataObj,datas)
			window.open(this.rootUrl+'/api/exportPasYearFeeMonitorList?'+$.param(ret)+'&token='+this.token)
		},
    	recoverQueryData(){
			...
			this.applytime = queryObj.deadlineStart && [new Date(queryObj.deadlineStart-0), new Date(queryObj.deadlineEnd-86390000)];
		  },
    	search(extra){
    		var ret = {}
    		
			if (this.applytime) {
				ret.deadlineStart = this.applytime[0] ? new Date(this.applytime[0]).getTime() : null;
				ret.deadlineEnd = this.applytime[1] ? new Date(this.applytime[1]).getTime() + 86390000 : null;
			}
      ...
      view(item){
      	this.$router.push({name: 'inventyearPayDetail', params: {id: item.caseSysNo,step:212}})
      },
    },
    filters: {
