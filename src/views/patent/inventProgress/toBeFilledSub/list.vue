	  ...
	  <div class="result">
        ...
        <thead>
          <tr class="thead">
			...
			<th class="smWidth">补正/审查意见截止日期</th>
        ...
        <tbody class="tr-color">
          <tr v-for="(item, index) in dataList" v-show="!loading && dataList.length">
			...
			<td class="nopay">{{item.supplementReviewDeadline|dateFormat}}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      view(item){
      	this.$router.push({name: 'toBeFilledSubDetail', params: {id: item.caseSysNo,step:29}})
      },
