	  ...
	  <div class="result">
        ...
        <thead>
          <tr class="thead">
			...
			<th class="smWidth">二期收款金额（元）</th>
			<th class="smWidth">缴纳申请费日期</th>
            <th class="smWidth">缴纳申请费金额（元）</th>
...
<script>
	...
	const workOrderListUrl = './api/findPasInvoiceList4ApplicatInvoiced';
	const dataObj = {//条件数据
		// contractNo:null,
		patentName:null,
		applicant: null,
  ...
  export default {
    ...
    methods: {
      ...
      view(item){
      	this.$router.push({name: 'mailedInvoiceDetail', params: {id: item.caseSysNo,type:item.patentType}})
      },
