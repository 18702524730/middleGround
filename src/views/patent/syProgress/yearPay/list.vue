	  ...
	  <div class="result">
        ...
        <tbody class="tr-color">
          <tr v-for="(item, index) in dataList" v-show="!loading && dataList.length">
			...
			<td class="nopay">{{item.whichYear}}</td>
  ...
  export default {
	...
	computed:{
		isexport(){
			...
			return arr.indexOf('pas_utility_flow_year_fee_paid_export')>-1
		}
	},
    mounted() {
    ...
    methods: {
      ...
      view(item){
      	this.$router.push({name: 'yearPayDetails', params: {id: item.caseSysNo,step:38}})
      },
    },
    filters: {
