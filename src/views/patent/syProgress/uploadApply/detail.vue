          ...
          <!-- 上传申请材料 -->
            <div class="basic_lists"  v-if="detailInfo.currentFlowStage==1 || detailInfo.pasFlowFeedbackMap[1]">
              <div class="basictitle">
                ...
                <el-button type="primary" @click="uploadMaterial('u','standby1')" v-if="detailInfo.currentFlowStage==1" class = 'fl'>上传</el-button>
              </div>
...
export default {
  ...
  methods: {
    ...
    torevise() {
      ...
      if(step==31){
        this.$router.push({name: "uploadApplyRevises",params: { state: this.caseSysNo,type:3,}});
      }
      if(step==32){
        this.$router.push({name: "uploadNoticeRevises",params: { state: this.caseSysNo,type:3,}});
      }
      if(step==33){
        this.$router.push({name: "applyFeeRevises",params: { state: this.caseSysNo,type:3,}});
      }
      if(step==34){
        this.$router.push({name: "firstAuditRevises",params: { state: this.caseSysNo,type:3,}});
      }
      if(step==35){
        this.$router.push({name: "toBeFilledRevises",params: { state: this.caseSysNo,type:3,}});
      }
      if(step==36){
        this.$router.push({name: "rejectedRevises",params: { state: this.caseSysNo,type:3,}});
      }
      if(step==37){
        this.$router.push({name: "bdfPayRevises",params: { state: this.caseSysNo,type:3,}});
      }
      if(step==38){
        this.$router.push({name: "yearPayRevises",params: { state: this.caseSysNo,type:3,}});
      }
