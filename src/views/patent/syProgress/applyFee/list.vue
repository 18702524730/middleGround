	  ...
	  <div class="result">
        ...
        <thead>
          <tr class="thead">
			...
			<th class="smWidth">申请号</th>
			<th class="comWidth">申请日</th>
			<th class="comWidth">申请费缴纳截止日期</th>
        ...
        <tbody class="tr-color">
          <tr v-for="(item, index) in dataList" v-show="!loading && dataList.length">
			...
			<td class="nopay">{{item.applicatFeeDeadline|dateFormat}}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      view(item){
      	this.$router.push({name: 'applyFeeDetails', params: {id: item.caseSysNo,step:33}})
      },
