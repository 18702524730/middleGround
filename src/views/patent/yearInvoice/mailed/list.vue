	  ...
	  <div class="result">
        ...
        <thead>
          <tr class="thead">
			...
			<th class="smWidth">当年年费收款金额（元）</th>
			<th class="minWidth">服务费（元）</th>
            <th class="smWidth">当年年费收款时间</th>
			<th class="smWidth">缴费时间</th>
			<th class="minWidth">缴费金额（元）</th>
			<th class="smWidth">官方黄票号</th>
			<th class="minWidth">制单</th>
			<th class="minWidth">电子/纸质发票</th>
			<th class="smWidth">当年年费官票号</th>
			<th class="smWidth">当年年费服务费票号</th>
        ...
        <tbody class="tr-color">
          <tr v-for="(item, index) in dataList" v-show="!loading && dataList.length">
            ...
            <td>{{item.phone}}</td>
			<td  class="alignc">{{item.whichYear}}</td>
            <td>{{item.yearFeeTurnInType|yearFeeTypeFormat}}</td>
...
<script>
	...
	const workOrderListUrl = './api/findPasInvoiceList4YearInvoiced';
	const dataObj = {//条件数据
		// contractNo:null,
		patentName:null,
		applicant: null,
  ...
  export default {
    ...
    methods: {
      ...
      view(item){
      	this.$router.push({name: 'mailedyearInvoiceDetail', params: {id: item.caseSysNo,type:item.patentType}})
      },
    ...
    filters: {
	  ...
	  yearFeeTypeFormat:Filters.yearFeeTurnInTypeFormat
    },
    watch:{
    	/*$route(to, from) {
